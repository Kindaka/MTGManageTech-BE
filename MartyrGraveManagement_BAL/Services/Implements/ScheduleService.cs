...
namespace MartyrGraveManagement_BAL.Services.Implements
{
    public class ScheduleService : IScheduleService
    {
        ...
        public async Task<List<string>> CreateSchedule(List<CreateScheduleDTORequest> requests)
        {
            ...
            foreach (var request in requests)
            {
                ...
                // 6. Kiểm tra số lượng Task trong Slot cho cùng một ngày
                var existingTasksInSlot = await _unitOfWork.ScheduleRepository.GetAsync(
                    s => s.Date.Date == request.Date.Date && s.SlotId == request.SlotId
                );

                if (existingTasksInSlot.Count() >= 3)
                {
                    results.Add($"Slot {slot.SlotName} vào ngày {request.Date.ToShortDateString()} đã đạt đến giới hạn tối đa là 3 Task.");
                    continue;
                }
...
>>>>>>> Stashed changes
        ...
        public async Task<string> UpdateSchedule(int scheduleId, UpdateScheduleDTORequest request)
        {
            ...
            var existingTasksInSlot = await _unitOfWork.ScheduleRepository.GetAsync(
                s => s.Date.Date == request.Date.Date && s.SlotId == request.SlotId && s.ScheduleId != scheduleId
            );
